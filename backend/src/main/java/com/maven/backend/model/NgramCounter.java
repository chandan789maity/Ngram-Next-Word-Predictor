...
public class NgramCounter
{
    ...
    public String generateNextWord(String[] ngram) {
        ...
        // Recursive case
        NgramCounter nextNgramCounter = map.get(ngram[ngram.length - level]);
        if (nextNgramCounter == null) {
            // Handle the case where no next NgramCounter is available
            return "UNKNOWN"; // or any other default behavior
        }
        return nextNgramCounter.generateNextWord(ngram);
    }

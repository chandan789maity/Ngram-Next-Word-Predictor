...
public class Ngram
{
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            try {
                System.out.println("Enter context words (space-separated) or 'exit' to quit:");
                ...
                // Check if the context length is correct
                if (context.length != n.n - 1) {
                    System.out.println("Error: Context length must be " + (n.n - 1));
                    continue;
                }
    
                // Predict the next word based on the context
                String nextWord = n.predictNextWord(context);
    ...
    public String predictNextWord(String[] context) {
        if (context.length != n-1) {
            throw new IllegalArgumentException("Context length must be " + (n-1));
        }
    
        // Prepare the words array with context
        String[] words = new String[n];
        System.arraycopy(context, 0, words, 0, context.length);
        words[n-1] = "???"; // Placeholder for the predicted word
    
        // Generate the next word based on the current context
        String nextWord = ngc.generateNextWord(words);
